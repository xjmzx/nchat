...
.PHONY: help deps dev build install uninstall check test clean icons version install-guard
...
install: install-guard $(TAURI_BIN)
	...
	sed '1s|viewBox="[^"]*"|viewBox="$(LINUX_VIEWBOX)"|' icon.svg > $(ICONDIR)/nchat.svg
	chmod 0644 $(ICONDIR)/nchat.svg
...
uninstall:
	...
	@# install rebuilt the cache with nchat.svg in it; left alone, it keeps
