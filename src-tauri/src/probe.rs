// Relay connectivity probe — carried over from nping, the suite's relay tester.
//
// nchat keeps it because a messenger that cannot tell you *why* a relay is not
// carrying your messages is only half a tool. The same three checks run here:
//
...
//
// Blocking I/O bounded by explicit timeouts, run on a blocking thread so many
// relays probe concurrently without stalling the UI.
...
#[tauri::command]
pub async fn probe_relay(url: String) -> RelayProbe {
